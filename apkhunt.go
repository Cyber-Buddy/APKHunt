...
func checkBinary(binary string) bool {
	// Use the "which" command to check if a binary exists
	_, err := exec.Command("which", binary).Output()

	// Check the return value of the command
	if err != nil {
		// If the return value is not 0, the binary does not exist
	...
	// If the return value is 0, the binary exists
	return true
}
