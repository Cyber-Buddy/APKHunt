...
func APKHunt_core(apkpath string) {
	...
	for _, sources_file := range files {
		if filepath.Ext(sources_file) == ".java" {
			...
			if err != nil {
				control_grep_error(err, cmd_and_pkg_packageObfuscate, "") //fmt.Println("- Obfuscated Code blocks have not been observed")
			}
			cmd_and_pkg_importObfuscate, err := exec.Command("grep", "-nr", "-F", "import com.a.", sources_file).CombinedOutput()
			if err != nil {
				control_grep_error(err, cmd_and_pkg_importObfuscate, "") //fmt.Println("- Obfuscated Code blocks have not been observed")
			}
			cmd_and_pkg_classObfuscate, err := exec.Command("grep", "-nr", "-F", "class a$b", sources_file).CombinedOutput()
			if err != nil {
				control_grep_error(err, cmd_and_pkg_classObfuscate, "") //fmt.Println("- Obfuscated Code blocks have not been observed")
			}
